...
function panel_locacion() {
	document
		.getElementById("LATITUD")
		.setAttribute("value", PANEL.telescopio.latitud);
	document
		.getElementById("LONGITUD")
		.setAttribute("value", PANEL.telescopio.longitud);
	document
		.getElementById("ALTITUD")
		.setAttribute("value", PANEL.telescopio.altitud);
}

// actualiza la lista de tipos de elementos celeste
async function panel_tipo_objeto_celeste() {
...
async function conectar() {
	...
	try {
		...
		document
			.getElementById("LATITUD")
			.setAttribute("value", datos.location.latitude);
		document
			.getElementById("LONGITUD")
			.setAttribute("value", datos.location.longitude);
		document
			.getElementById("ALTITUD")
			.setAttribute("value", datos.location.altitude);
	} catch (error) {
		//TODO mostrar el error en pantalla y link a instrucciones para setear stellarium
		console.log("error");
	}
...
async function listaObjetosCelestes() {
	const url =
		"http://localhost:8090/api/objects/listobjectsbytype?type=" +
		PANEL.objeto.tipo;
...
async function datosObjeto() {
	...
	try {
		...
		info +=
			'<dt class="col-4">Visible</dt> <dd class="col-8">' + visible + "</dd>";
		info +=
			'<dt class="col-4">AZ</dt> <dd class="col-8">' +
			datos.azimuth.toFixed(3) +
			"&#176;</dd>";
		info +=
			'<dt class="col-4">EL</dt> <dd class="col-8">' +
			datos.altitude.toFixed(3) +
			"&#176;</dd>";
		if ("illumination" in datos)
			info +=
				'<dt class="col-4">Iluminado</dt> <dd class="col-8">' +
				datos.illumination.toFixed(2) +
				" %</dd>";
		if ("distance" in datos)
			info +=
				'<dt class="col-4">Distancia</dt> <dd class="col-8">' +
				datos.distance.toFixed(4) +
				" AU</dd>";
		if ("distance" in datos) {
...
function angulo() {
	PANEL.telescopio.angulo = document.querySelector(
		'input[name="ANGULO"]:checked'
	).value;
...
function modoNoche() {
	...
	html.classList.contains("modo_noche")
		? html.removeAttribute("class", "modo_noche")
...
SEGUIMIENTO.addEventListener("click", () => {
	PANEL.serial.seguimiento =
		PANEL.serial.seguimiento == 0
			? setInterval(datosObjeto, 200)
			: PANEL.serial.seguimiento;
	PANEL.serial.intervalo =
		PANEL.serial.intervalo == 0
			? setInterval(enviarComandoTelescopio, 200)
			: PANEL.serial.intervalo;
});
